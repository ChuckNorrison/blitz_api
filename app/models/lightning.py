...
class InvoiceHTLC(BaseModel):
    ...
    def from_lnd_grpc(cls, h) -> "InvoiceHTLC":
        def _crecords(recs):
            entries_list = []
            for r in recs:
                entries_list.append(CustomRecordsEntry.from_lnd_grpc(r))
            return entries_list
...
""",
    ...
    def from_lnd_grpc(cls, i) -> "Invoice":
        def _route_hints(hints):
            hints_list = []
            for h in hints:
                hints_list.append(RouteHint.from_lnd_grpc((h)))
            return hints_list

        def _htlcs(htlcs):
            htlc_list = []
            for h in htlcs:
                htlc_list.append(InvoiceHTLC.from_lnd_grpc(h))
            return htlc_list

        def _features(features):
            features_list = []
            for k in features:
                features_list.append(FeaturesEntry.from_lnd_grpc(k, features[k]))
...
class Route(BaseModel):
    ...
    def from_lnd_grpc(cls, r):
        def _crecords(recs):
            records = []
            for r in recs:
                records.append(CustomRecordsEntry(r))
            return records

        def _get_hops(hops) -> List[Hop]:
            hops = []
            for h in hops:
                hops.append(Hop.from_lnd_grpc(h))
            return hops
...
class Payment(BaseModel):
    ...
    def from_lnd_grpc(cls, p) -> "Payment":
        def _get_attempts(attempts):
            attempts = []
            for a in attempts:
                attempts.append(HTLCAttempt.from_lnd_grpc(a))
            return attempts
